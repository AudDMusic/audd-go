# audd-go — Official Go SDK for AudD
...
Official Go SDK for [AudD](https://audd.io) — music recognition from a short audio clip, a long audio file, or a live stream.
...
```go
client := audd.NewClient("aud_xxx_your_token")
```
...
http.HandleFunc("/audd/callback", func(w http.ResponseWriter, r *http.Request) {
    ...
    case payload.IsResult():
        for _, e := range payload.Result.Results {
            fmt.Println("matched:", e.Artist, "—", e.Title)
        }
    case payload.IsNotification():
        fmt.Println("notification:", payload.Notification.NotificationMessage)
    }
})
```
...
category := streams.DeriveLongpollCategory(1) // computed locally; no round-trip
ch, err := streams.LongpollContext(ctx, category, nil)
if err != nil { log.Fatal(err) }

for ev := range ch {
    if ev.Err != nil { log.Fatal(ev.Err) }
    fmt.Println(ev.Body)
}
```

For browser widgets, embedded UIs, or anywhere you need to consume a category
without leaking the API token, use the tokenless variant:

```go
consumer := audd.NewLongpollConsumer(category)
defer consumer.Close()

for ev := range consumer.Iterate(nil) {
    if ev.Err != nil { log.Fatal(ev.Err) }
    fmt.Println(ev.Body)
}
```
