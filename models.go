...
// metadata block when the user requested it via Return. Empty string if not
// available.
...
//     user requested that provider via Return. Direct = no redirect, faster.
...
// StreamCallbackResultEntry is one match in a callback's results array.
type StreamCallbackResultEntry struct {
...
// StreamCallbackResult is a recognition-result variant of a stream callback.
type StreamCallbackResult struct {
	RadioID    int                         `json:"radio_id"`
	Timestamp  string                      `json:"timestamp,omitempty"`
	PlayLength int                         `json:"play_length,omitempty"`
	Results    []StreamCallbackResultEntry `json:"results"`

	Extras      map[string]json.RawMessage `json:"-"`
	RawResponse json.RawMessage            `json:"-"`
}

// StreamCallbackNotification is a notification variant of a stream callback
// (e.g. "stream stopped", "can't connect").
...
// StreamCallbackPayload is the typed wrapper around a callback POST body.
// Exactly one of Result / Notification is set.
type StreamCallbackPayload struct {
	Result       *StreamCallbackResult
	Notification *StreamCallbackNotification
	Time         int
	Raw          json.RawMessage
}

// IsResult reports whether this payload carries a recognition result.
func (p *StreamCallbackPayload) IsResult() bool { return p.Result != nil }

// IsNotification reports whether this payload carries a stream notification.
func (p *StreamCallbackPayload) IsNotification() bool { return p.Notification != nil }
...
var streamCallbackResultEntryKnownKeys = map[string]bool{
...
func (e *StreamCallbackResultEntry) UnmarshalJSON(data []byte) error {
	type alias StreamCallbackResultEntry
	if err := json.Unmarshal(data, (*alias)(e)); err != nil {
		return err
	}
	extras, err := extractExtras(data, streamCallbackResultEntryKnownKeys)
	if err != nil {
		return err
	}
	e.Extras = extras
	e.RawResponse = append(e.RawResponse[:0], data...)
	return nil
}

var streamCallbackResultKnownKeys = map[string]bool{
	"radio_id": true, "timestamp": true, "play_length": true, "results": true,
}

func (r *StreamCallbackResult) UnmarshalJSON(data []byte) error {
	type alias StreamCallbackResult
	if err := json.Unmarshal(data, (*alias)(r)); err != nil {
		return err
	}
	extras, err := extractExtras(data, streamCallbackResultKnownKeys)
	if err != nil {
		return err
	}
	r.Extras = extras
