...
func TestStreamCallbackPayload_TypeFlags(t *testing.T) {
	p1 := &StreamCallbackPayload{Result: &StreamCallbackResult{}}
	assert.True(t, p1.IsResult())
	assert.False(t, p1.IsNotification())

	p2 := &StreamCallbackPayload{Notification: &StreamCallbackNotification{}}
	assert.True(t, p2.IsNotification())
	assert.False(t, p2.IsResult())
}
