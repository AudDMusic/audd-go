...
func TestStreams_SetCallbackUrl_ExtraParametersPropagated(t *testing.T) {
	var seenFoo, seenURL string
	c, _ := newMockClient(t, func(w http.ResponseWriter, r *http.Request) {
		require.NoError(t, r.ParseMultipartForm(1<<20))
	...
	require.NoError(t, c.Streams().SetCallbackUrl("https://example.com/cb",
		&SetCallbackUrlOptions{
			ExtraParameters: map[string]string{"foo": "bar", "url": "ignored"},
		}))
	assert.Equal(t, "bar", seenFoo)
	assert.Equal(t, "https://example.com/cb", seenURL, "typed url must win over ExtraParameters")
}

func TestStreams_Add_ExtraParametersPropagated(t *testing.T) {
	var seenFoo, seenURL, seenRadio string
	c, _ := newMockClient(t, func(w http.ResponseWriter, r *http.Request) {
		require.NoError(t, r.ParseMultipartForm(1<<20))
		seenFoo = r.FormValue("foo")
		seenURL = r.FormValue("url")
	...
	require.NoError(t, c.Streams().Add(AddStreamRequest{
		URL:             "twitch:somechannel",
		RadioID:         7,
		ExtraParameters: map[string]string{"foo": "bar", "url": "ignored"},
	}))
	assert.Equal(t, "bar", seenFoo)
	assert.Equal(t, "twitch:somechannel", seenURL, "typed URL must win over ExtraParameters")
