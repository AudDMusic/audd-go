...
func TestStreams_ParseCallback_Result(t *testing.T) {
	c := NewClient("test")
	defer func() { _ = c.Close() }()
	...
	p, err := c.Streams().ParseCallback(body)
	require.NoError(t, err)
...
func TestStreams_ParseCallback_Notification(t *testing.T) {
	c := NewClient("test")
	defer func() { _ = c.Close() }()
...
func TestStreams_Longpoll_PreflightSkippable(t *testing.T) {
	...
	defer cancel()
	ch, err := c.Streams().LongpollContext(ctx, "cat", &LongpollOptions{SkipCallbackCheck: true, Timeout: 50})
	require.NoError(t, err)
...
func TestStreams_Longpoll_PreflightOK_PassesThrough(t *testing.T) {
	c, _ := newMockClient(t, func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/getCallbackUrl/":
			_, _ = w.Write([]byte(`{"status":"success","result":"https://example.com/cb"}`))
	...
	defer cancel()
	ch, err := c.Streams().LongpollContext(ctx, "cat", nil)
	require.NoError(t, err)
	select {
	case ev := <-ch:
...
func TestStreams_Longpoll_HTTP500_ClosesWithError(t *testing.T) {
	...
	// 500 with non-JSON body in a longpoll-event slot returns a serialization error
	// (because the longpoll consumer in StreamsClient checks JSONBody). Either way,
	// the channel must yield an error rather than spin.
	assert.True(t, errors.Is(ev.Err, ErrSerialization) || errors.Is(ev.Err, ErrServer))
}
